...
def score_signal_detailed(evt: SignalEvent, tf: TimeframeCfg) -> dict:
    """Return component breakdown for frontend VCPSignal format."""
    eps = 1e-9
    c1, c2, c3 = evt.c1_range_pct, evt.c2_range_pct, evt.c3_range_pct
    r12 = c1 / max(c2, eps)
    r23 = c2 / max(c3, eps)
    ...
    total = int(round(clamp(contraction + trend + trigger + risk_liq, 0.0, 100.0)))
    valid_vcp = c1 > c2 and c2 > c3
