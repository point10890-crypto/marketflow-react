...
## 11. 전역 리스킨 + 모바일 (2026-08-22 후속 2)

- **적용 방식**: 페이지 JSX 무수정. `.claw-theme` 스코프의 CSS 테마 레이어가 모든 페이지가 공유하는 패턴만 재정의 — 다크 카드 배경(`bg-[#13151f]`·`bg-[#1c1c1e]` 등 8종 → `#110d10`), 둥근 카드의 흰 헤어라인 → 붉은 헤어라인, 악센트 단어(Tailwind 그라디언트·인라인 그라디언트 모두) → 브랜드 레드 그라디언트, 사이드바 활성 항목 레드 레일(`claw-nav-active`), 하단 탭 활성 코랄(`claw-tab-active`), 검색 필·스크롤바·섹션 라벨.
- **고정 타이틀**: 상단 배너는 일반 흐름(콘텐츠와 함께 스크롤, 크기 애니메이션 없음). 접힌 한 줄 바는 높이 0 sticky 래퍼 안의 overlay 로 transform/opacity 만 전환(블러·레이아웃 전환 없음 — 2026-08-23 스크롤 버벅임 수정). 모바일(≤640)은 반경 14·상태 문장 숨김.
- **모바일 세로**: 셸 패딩 10px, `rounded-3xl`→18px, 풀 히어로 축소(마스코트 84, 헤드라인 26px). 375×812 에서 Summary·Claw LIVE 가로 스크롤 0, 상태 타일 2열 확인.
- **의도적 제외**: 등급/KRX/성공·경고 의미색, W Pattern 전체화면 캔버스 페이지(브랜드 바 제외).
- **후속(선택)**: 페이지 내부 고유 컴포넌트(게이지 색, 섹션 아이콘 색)의 개별 리스킨, 라이트 모드 없음(다크 단일).
