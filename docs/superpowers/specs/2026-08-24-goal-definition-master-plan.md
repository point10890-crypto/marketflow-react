...
### 남은 것
1. miniPC git 미푸시 커밋 6개 push (miniPC 쪽 선행) → 본PC P1 커밋/푸시 → miniPC pull
2. 8/16 vs 8/24 리포트 diff — 게이트 강도 악화 원인 규명
3. miniPC 이더넷 고정 IP(55.x) 복원 — 현재 APIPA(169.254.144.42) 임시 경로, 재부팅 시 주소 변동 가능 (시스템 설정 변경이라 사용자 수행/승인 필요)
