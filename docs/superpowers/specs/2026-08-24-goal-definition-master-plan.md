...
### 남은 것
1. G5-①② — 23:30 night 사이클에서 `refresh_intelligence` 실행·top3 갱신 확인 (진행 중)
2. lab V1 게이트를 운영 게이트와 정합(양국면 차단 파라미터화) — 소규모 후속
3. `stash@{0}` 드리프트 작업본(23파일) 검토 → 채택분 정식 커밋 (특히 국면별 PF, 확장 러너, regime 수정)
4. miniPC 이더넷 고정 IP(55.x) 복원 — 현재 APIPA(169.254.144.42), 재부팅에도 유지됐으나 보장 없음 (사용자 수행/승인 필요)
