...
def _esc(value: Any) -> str:
    """Escape dynamic Telegram HTML text while leaving template tags intact."""
...
def _regime_line(reg: dict[str, Any]) -> str:
    parts = [f"레짐 <b>{_esc(reg.get('regime'))}</b>"]
    if reg.get('gate_score') is not None:
        parts.append(f"gate {_esc(reg.get('gate_status'))} {_esc(reg.get('gate_score'))}점")
...
def halt_message(reg: dict[str, Any], ts: str | None = None) -> str:
    reasons = '\n'.join(f" - {_esc(r)}" for r in reg.get('reasons') or []) or ' - (사유 없음)'
    observed = datetime.fromisoformat(ts) if ts else datetime.now()
    return (f"⏸ <b>검출 보류 (HALT)</b> {observed.strftime('%H:%M')}\n"
            f"사유:\n{reasons}\n"
...
def event_message(events: list[dict[str, Any]], reg: dict[str, Any]) -> str:
    ...
    for e in events[:5]:
        lab = _esc(TYPE_LABEL.get(e['type'], e['type']))
        g = GRADE_MARK.get(e.get('grade_to') or e.get('grade') or '', '')
        try:
            event_at = datetime.fromisoformat(str(e.get('ts') or '')).strftime('%m-%d %H:%M')
        except (TypeError, ValueError):
            event_at = '-- --:--'
        trans = ''
        if e['type'] in ('LEADER_UPGRADE', 'LEADER_DROP'):
            trans = f" {_esc(e.get('grade_from') or '–')}→{_esc(e.get('grade_to') or '–')}"
        # Keep the original event time visible. This matters when a transport
        # outage near the close is drained on the next trading day.
        lines.append(f"[{lab}]{trans} {g} {event_at} · {_esc(e['name'])} {_esc(e['code'])} · {_esc(e.get('score', 0))}점")
...
def morning_message(snap: dict[str, Any], reg: dict[str, Any], prev_events: list[dict[str, Any]]) -> str:
    ...
    if by_type:
        lines.append(' ' + ' · '.join(f"{_esc(TYPE_LABEL.get(k, k))} {v}" for k, v in by_type.items()))
    leaders = [r for r in snap.get('rows') or [] if r.get('grade') in ('S', 'A')][:6]
    lines.append('')
    lines.append(f"<b>감시 목록</b> (마지막 스냅샷 {_esc(snap.get('ts', '')[11:16])}, {_esc(snap.get('source'))})")
    for r in leaders:
        lines.append(f" {GRADE_MARK.get(r['grade'], '')} {_esc(r['name'])} {_esc(r['code'])} · {_esc(r['score'])}점 · {_pct(r['chg'])}")
    return '\n'.join(lines)
...
def close_message(snap: dict[str, Any], reg: dict[str, Any], events: list[dict[str, Any]], stats: dict[str, Any]) -> str:
    ...
    for r in leaders[:8]:
        lines.append(f" {GRADE_MARK.get(r['grade'], '')} {_esc(r['name'])} {_esc(r['code'])} {_pct(r['chg'])} · {_esc(r['score'])}점 · {r['trval_eok']:,.0f}억")
    drops = [e for e in events if e['type'] == 'LEADER_DROP']
    if drops:
        lines.append('<b>이탈</b> ' + ' · '.join(f"{_esc(e['name'])} {_pct(e.get('chg'))}" for e in drops[:5]))
    lines.append('')
