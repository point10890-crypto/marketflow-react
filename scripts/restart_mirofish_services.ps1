...
Write-Host "=== Flask :5003 재시작 ==="
$flaskPids = Get-NetTCPConnection -LocalPort 5003 -State Listen -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique
...
$newFlask = Get-NetTCPConnection -LocalPort 5003 -State Listen -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique
...
Write-Host "=== 응답 검증 ==="
try {
    $r1 = Invoke-WebRequest -Uri http://localhost:5003/api/admin/mirofish/status -UseBasicParsing -TimeoutSec 5
    Write-Host "  Flask 5003 HTTP $($r1.StatusCode)"
} catch {
    Write-Host "  Flask 5003 status: $($_.Exception.Response.StatusCode.value__)"
}
