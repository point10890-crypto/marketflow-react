    ...
    return (
        <section className={`claw-card-in rounded-2xl border p-4 sm:rounded-3xl sm:p-5 lg:col-span-12 ${tone(quality, scorecards)}`}>
            <div className="flex flex-wrap items-center gap-2">
                <h2 className="dash-section-title flex items-center gap-2 text-[15px] font-bold text-white">
                    <i className="fas fa-clipboard-check text-[13px] text-[#ff8a6b]" />관측·검증 하네스
                </h2>
                <span className={`rounded-full px-2 py-1 text-[12px] font-extrabold ${state.cls}`}>{state.text}</span>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-[1fr_1fr_1fr_1.3fr]">
                ...
                <div className="min-w-0 rounded-xl border border-white/[0.05] bg-black/15 px-3 py-2.5">
                    <div className="text-[12px] font-bold uppercase tracking-wider text-gray-400">최근 Signal</div>
                    {latest ? (
                        <div className="mt-1 min-w-0 text-[12px]">
                            <div className="flex min-w-0 items-center gap-2">
                                <b className="min-w-0 flex-1 truncate text-white">{latest.name || latest.code}</b>
                                <span className="shrink-0 font-mono text-[11px] text-gray-400">{hhmm(latest.opened_at)}</span>
                            </div>
                            <span className="mt-1 inline-flex max-w-full truncate rounded bg-white/[0.06] px-1.5 py-0.5 font-mono text-[12px] text-gray-400">{latest.trigger_type}</span>
                        </div>
                    ) : <p className="mt-1 text-[12px] text-gray-400">새 관측을 기다리는 중</p>}
                </div>
            </div>
            ...
            {horizons.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-1.5 border-t border-white/[0.05] pt-3">
                    ...
                    {scorecards?.insufficient && <span className="self-center text-[11px] text-gray-400">정책 미적용 · {scorecards.insufficient_reason || '표본 부족'}</span>}
                </div>
            )}
        </section>
    );
...
function Metric({ label, value, detail, toneClass }: { label: string; value: number; detail: string; toneClass: string }) {
    return (
        <div className="rounded-xl border border-white/[0.05] bg-black/15 px-3 py-2.5">
            <div className="text-[12px] font-bold uppercase tracking-wider text-gray-400">{label}</div>
            <div className={`mt-1 font-mono text-xl font-black tabular-nums ${toneClass}`}>{value.toLocaleString('ko-KR')}</div>
            <div className="mt-0.5 text-[12px] text-gray-400">{detail}</div>
        </div>
    );
}
