...
export default function ClawBriefsCard({ data }: { data: ClawOverview }) {
    ...
    return (
        <section className="dash-panel market-panel claw-card-in rounded-2xl border border-white/[0.06] bg-[#13151f] p-4 sm:rounded-3xl sm:p-5 lg:col-span-8">
            <h2 className="dash-section-title mb-3 flex items-center gap-2 text-[15px] font-bold text-white">
                ...
                브리핑
                <span className="ml-auto text-[11px] font-medium text-gray-400">{list.length ? `${list.length}건` : ''}</span>
            </h2>
            {list.length > 0 && (
                    ...
                    {list.map((b, i) => (
                        <button key={b.digest || i} type="button" role="tab" aria-selected={i === tab} onClick={() => setTab(i)}
                            ...
                            <span className={b.delivered ? 'text-teal-300' : 'text-amber-300'}>
                                <i className={`fas ${b.delivered ? 'fa-check' : 'fa-flask'} text-[12px]`} /> {b.delivered ? '발송' : 'dry-run'}
                            </span>
                        </button>
                    ))}
                </div>
            ...
            <div className="whitespace-pre-wrap break-keep rounded-xl rounded-bl-[4px] border border-white/[0.06] bg-[#181b27] px-3.5 py-3 text-[13px] leading-relaxed text-gray-200 sm:max-h-72 sm:overflow-auto">
                {cur ? renderTelegramText(cur.text) : <span className="text-gray-400">아직 브리핑이 없어요 — 08:20 조간 브리핑부터 차곡차곡 쌓입니다</span>}
            </div>
        </section>
    );
}
