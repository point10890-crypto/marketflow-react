...
export default function ClawEventsCard({ data }: { data: ClawOverview }) {
    ...
    return (
        <section className="dash-panel market-panel claw-card-in rounded-2xl border border-white/[0.06] bg-[#13151f] p-4 sm:rounded-3xl sm:p-5 lg:col-span-5">
            <h2 className="dash-section-title mb-3 flex items-center gap-2 text-[15px] font-bold text-white">
                ...
                <span className="ml-auto text-[11px] font-medium text-gray-400">
                    {data.loop.market_open ? '실시간' : <span className="rounded-full bg-white/[0.06] px-2 py-0.5 text-[12px] font-bold text-gray-300">전 세션 이벤트</span>}
                </span>
            </h2>
            <div className="mb-2.5 flex flex-wrap gap-1.5">
                {FILTERS.map(f => (
                    <button key={f.key} type="button" onClick={() => setFilter(f.key)} aria-pressed={filter === f.key}
                        ...
                        {f.label}{f.key !== 'ALL' && data.events.counts[f.key] ? <span className="ml-1 text-gray-400">{data.events.counts[f.key]}</span> : null}
                    </button>
                ))}
            </div>
            {items.length === 0 ? (
                    ...
                    {items.map((e, i) => {
                        ...
                        return (
                            <li key={`${e.ts}-${e.type}-${e.code}-${i}`} className={`grid grid-cols-[42px_44px_minmax(0,1fr)_16px] items-center gap-2 py-2.5 text-[13px] sm:grid-cols-[44px_46px_minmax(0,1fr)_auto_16px] ${halt ? 'rounded-lg bg-amber-500/[0.06] px-1.5' : ''}`}>
                                <span className="font-mono text-[12px] tabular-nums text-gray-400">{hhmm(e.ts)}</span>
                                <span className={`rounded px-1.5 py-0.5 text-center text-[12px] font-bold ${chip.cls}`}>{chip.label}</span>
                                <span className="min-w-0 truncate text-gray-100">
                                    ...
                                    {e.code && <span className="ml-1.5 font-mono text-[11px] text-gray-400">{e.code}</span>}
                                </span>
