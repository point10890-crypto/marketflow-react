...
export default function QuickActionsFooter() {
    ...
    return (
        <section className="rounded-xl border border-violet-300/15 bg-slate-950/60 p-4 shadow-[0_18px_70px_rgba(167,139,250,0.10)]">
            <header className="flex items-center justify-between">
                <div>
                    <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.22em] text-violet-200/70">
                        <i className="fas fa-bolt text-violet-300" />
                        Quick Actions
                    </div>
                    <h3 className="mt-1 text-base font-black text-white">빠른 액션 콘솔</h3>
                </div>
            </header>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
                {actions.map((action) => {
                    ...
                    const tone =
                        ...
                        'border-white/10 bg-black/30 text-slate-200 hover:border-violet-300/30 hover:bg-violet-300/[0.08]';
                    return (
                        <button
                            key={action.key}
                            type="button"
                            ...
                            onClick={() => exec(action)}
                            className={`group rounded-lg border px-3 py-2 text-left text-[11px] font-bold transition-colors disabled:cursor-wait ${tone}`}
                            title={action.description}
                        >
                            <div className="flex items-center justify-between">
                                <span className="font-black">{action.emoji} {action.label}</span>
                                {state === 'running' && (
