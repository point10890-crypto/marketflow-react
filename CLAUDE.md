...
### 절대 규칙
1. **경로**: Windows 경로(`C:\...`) 사용 금지 → 항상 MINGW 경로: `/c/bitman_marketfloww`
...
### 경로 설정
```python
Config.BASE_DIR     = /c/bitman_marketfloww          # __file__ 기반
Config.DATA_DIR     = /c/bitman_marketfloww/data
Config.PYTHON_PATH  = /c/bitman_marketfloww/.venv/Scripts/python.exe  # venv 우선
Config.LOG_DIR      = /c/bitman_marketfloww/logs
```
...
- 100점 채점 엔진 (거래대금30 + 모멘텀25 + 수급25 + 급증10 + 섹터10)
- Flask 백그라운드 폴링 워커 (장중 5초 간격)
- S등급 텔레그램 알림 (5분 쿨다운)
- 프론트엔드: KrLeadingStocksPage + Summary 통합 카드

**코드 리팩터링:**
- app/utils/paths.py: 중앙 경로 상수 (5개 파일 중복 제거)
- app/utils/safety.py: safe_float/safe_int/safe_str 통합
- common.py: print() → logger 전환, bare except 수정
- .claude/skills/: 6개 스킬 등록 (deploy, restart-flask, test-endpoints 등)
- screener_test/ 정리 (Python 파일 삭제, SPEC/WORKFLOW 유지)
- VCP dates 엔드포인트 누락 수정 (KR/US/Crypto)

**사용자 승인 시스템:**
- 신규 가입 → pending → 관리자 승인 후 사용 가능
